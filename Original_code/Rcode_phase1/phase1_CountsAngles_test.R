# phase1_CountsAngles_test.R: JAB 2018/07/12, edited WSK 2018/07/25
#
# CONTENT:
# LIBRARIES, PATHS 
...
#    - phase1_PixelReadingPilatus.R, including consistency checks there
#
# INPUT: 
...
dirOut <- file.path(location, "Output_phase1", "Phase1_CountsAngles", "Misc")
if (Layout$name == "Pilatus"){ 
  dirOut <- file.path(location, "Output_phase1", "Phase1_CountsAngles", "Pilatus")
}

if (substring(Layout$name,1,11) == "PerkinElmer"){
  dirOut <- file.path(location, "Output_phase1", "Phase1_CountsAngles", paste(Layout$name,Layout$date, sep=""))
}
...
pdf(file.path(dirOut,  paste("layoutDamaged_", Layout$name, ".pdf", sep="")))
