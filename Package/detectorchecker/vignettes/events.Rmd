---
title: "Events"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Damaged layout
```{r}
pilatus_layout <- create_module("Pilatus")

file_path <- "../tests/testthat/dead_pix/Pilatus/badpixel_mask.tif"

pilatus_layout <- load_pix_matrix(layout = pilatus_layout, file_path = file_path)

plot_layout_damaged(layout = pilatus_layout)
```
```{r}

pilatus_layout_temp <- find_clumps(pilatus_layout)

plot_events(pilatus_layout_temp, caption = FALSE, incl_event_list = NA)
```

### Classifying and plotting events (Selecting events for analysis)
```{r}

pilatus_layout <- find_clumps(pilatus_layout)

plot_events(pilatus_layout)

plot_pixels_events(pilatus_layout)

plot_events(pilatus_layout, incl_event_list = 4)

plot_pixels_events(pilatus_layout, incl_event_list = 4)

plot_events(pilatus_layout, incl_event_list = list(2,3,4,5,6,7,8,9))

plot_pixels_events(pilatus_layout, incl_event_list = list(2,3,4,5,6,7,8,9))

```

### Plotting events density (selecting modules too)
```{r}

pilatus_layout_temp <- find_clumps(pilatus_layout)

plot_events_density(pilatus_layout_temp, caption = FALSE, incl_event_list = NA)
```

```{r}
pilatus_layout_temp <- find_clumps(pilatus_layout, row=5, col=1)

plot_events_density(pilatus_layout_temp, incl_event_list = list(4))

```
### Plotting counts
```{r}
pilatus_layout_temp <- find_clumps(pilatus_layout)

plot_events_count(pilatus_layout_temp, caption = FALSE)

```


### Plotting arrows (selecting modules too)
```{r}

pilatus_layout <- find_clumps(pilatus_layout)

plot_events_arrows(pilatus_layout, incl_event_list = list(4))

pilatus_layout <- find_clumps(pilatus_layout, row=5, col=1)

plot_events_arrows(pilatus_layout, incl_event_list = list(4))
```
### Plotting KFGs

```{r}
pilatus_layout <- find_clumps(pilatus_layout)

plot_events_kfg(pilatus_layout, func = "Kinhom", incl_event_list = list(4))

```
