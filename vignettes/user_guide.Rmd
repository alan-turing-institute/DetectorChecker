...
If you have downloaded the package together with this vignette, then the vignette can be re-built.
```{r}
library(detectorchecker)
vignette("user-guide", package = "detectorchecker")
```
...
Otherwise a user-defined layout can be speficied by reading in an
appropriate text file as explained further below.

### Create detector object from available layouts and 
...
In the `Pilatus` detectors all horizontal gaps are of the same size, and all vertial gaps are of the same size. `Perkin Elmer` and `Excalibur` detectors have no gaps at all. However, it is possible to create layouts with more irregular gap sizes (see the comments below on user-defined layouts).
...
Slots 9 and 10 are calculated from the other slots; they contain the left and the right edge (column numbers) and the top and bottom edge (row numbers) of each module. If there are no gaps, such as in the `Perkin Elmer` layouts, then the left edge of a module will be adjacent to the right edge of the previous one in the same row of the grid and the bottom edge of a module will be adjecent to the top edge of the previous one.
...
Slot 13 is a consistency check. If the sizes of the modules add up to the correct numbers, the status is 0. Otherwise, it is 1 and suitable error message will be given. The remaining slots will be filled when the detector damage information is read in.
...
We illustrate this by two examples. The first example is a simple layout for a detector with photographic aspect ratio with no subdivisions. We stored the lines below as a plain text file under the name  layout_par_photographic.txt in a the subdirectory 
...
The second example is an irregular layout with varying module dimensions and varying gap sizes. We stored the lines below as a plain text file under the name  layout_par_irregular.txt in a the subdirectory 
...
To generate a detector object from user-defined parameters, we use a function reading the parameters from the text file as shown below for our examples. For your own user-defined detector you need to adjust the file_path to lead to the directory where you stored the parameter file.

```{r}
file_path <-  system.file("extdata", "user-defined", "photographic", "layout_par_photographic.txt", 
                          package ="detectorchecker")
user_def_detector_photographic <- readin_detector(file_path)
file_path <-  system.file("extdata", "user-defined", "irregular", "layout_par_irregular.txt", 
                          package ="detectorchecker")
...
A visualisation of the layout shows modules and gaps as shown below for two of prespecified layouts. The default is to plot a title, but we can override this by setting the parameter caption to `FALSE`. The same rule applies for all plots produced using this package.
...
The class for detectors considered admissible for this package is illustrated here.

![General layout with parameters](layoutDrawingPar.png)
...
The spatial locations of the damagedd pixels can be visualised.

```{r}
plot_pixels(detector = detector_pilatus)
```

Individual modules can be plotted separately. Here we plot the module located in the 4th column and 5th row of the module layout grid.

```{r}
plot_pixels(detector = detector_pilatus, col = 4, row = 5)
```

For the user-defined layout used above we can also display damaged pixels. If the location are stored in form of a tiff file this looks as follows.

```{r}
file_path <-  system.file("extdata", "user-defined", "photographic", "photographic.tif", package ="detectorchecker")
user_def_detector_photographic <- load_pix_matrix(detector = user_def_detector_photographic, file_path = file_path)
plot_pixels(detector = user_def_detector_photographic, caption = TRUE)
```

#### XML format

For `Perkin Elmer` detectors, damaged pixels locations are usually stored as `XML` files. Sample files for each of the three `Perkin Elmer` types are supplied with the package and can be loaded by specifying the location and file name. 

```{r}
detector_perkinfull <- create_detector("PerkinElmerFull")
file_path <-  system.file("extdata", "PerkinElmerFull", "BadPixelMap_t1.bpm.xml",
                          package = "detectorchecker")
detector_perkinfull <- load_pix_matrix(detector = detector_perkinfull, file_path = file_path)
plot_pixels(detector = detector_perkinfull)
plot_pixels(detector = detector_perkinfull, col = 11, row = 1)
```
...
file_path <-  system.file("extdata", "PerkinElmerRefurbished", "BadPixelMap_t1.bpm.xml", package = "detectorchecker")
detector_perkinrefurb <- load_pix_matrix(detector = detector_perkinrefurb, file_path = file_path)
...
```{r}
detector_perkinrefurb <- create_detector("PerkinElmerRefurbished")
file_path <-  system.file("extdata", "PerkinElmerRefurbished", "BadPixelMap_t2.bpm.xml", package = "detectorchecker")
detector_perkinrefurb <- load_pix_matrix(detector = detector_perkinrefurb, file_path = file_path)
plot_pixels(detector = detector_perkinrefurb)
```

This also works for a user-defined detector if the damaged pixel locations are stored in xml format. (In the example file the dead pixels locations were similated by an Poisson point pattern with inhomogenous density.)

```{r}
file_path <-  system.file("extdata", "user-defined", "photographic", "badpixelmap_photographic_inhom.xml", 
                          package ="detectorchecker")
user_def_detector_photographic <- load_pix_matrix(detector = user_def_detector_photographic, file_path = file_path)
plot_pixels(detector = user_def_detector_photographic, caption = TRUE)
```
...
```{r}
file_path <-  system.file("extdata", "user-defined", "irregular", "badpixelmap_irregular.xml", 
                          package ="detectorchecker")
user_def_detector_irregular <- load_pix_matrix(detector = user_def_detector_irregular, file_path = file_path)
...
plot_pixels_count(detector = detector_pilatus, row = 5, col = 12)
```
...
plot_pixels_arrows(detector = detector_perkinfull, row = 1, col = 1)
```
...
```{r}
plot_pixels_angles(detector = detector_pilatus, row = 5, col = 11)
plot_pixels_angles(detector = detector_pilatus, row = 5, col = 4)
```
...
```{r, fig.show='hide'}
plot_pixels_kfg(detector = detector_pilatus, func = "K", row = 5, col = 12)
plot_pixels_kfg(detector = detector_pilatus, func = "F", row = 5, col = 12)
plot_pixels_kfg(detector = detector_pilatus, func = "G", row = 5, col = 12)
```
...
```{r, fig.show='hide'}
plot_pixels_kfg(detector = detector_pilatus, func = "Kinhom", row = 5, col = 12)
plot_pixels_kfg(detector = detector_pilatus, func = "Finhom", row = 5, col = 12)
plot_pixels_kfg(detector = detector_pilatus, func = "Ginhom", row = 5, col = 12)
```
...
The differences between pixel and event level are demonstrated well in the `Perkin Elmer` example data sets, because they possess lines of damaged pixels. 
...
```{r}
plot_pixels(detector_perkinfull, row = 1, col = 11)
plot_events(detector_perkinfull_events, row = 1, col = 11, 
                   incl_event_list = incl_event_list)
```
...
For density plots the optional adjust paramteer can be omitted (default=0.5) or changed to another positive value. Individual modules can be selected.

```{r}
detectorchecker::plot_events_density(detector_perkinfull_events, 
                                     incl_event_list = incl_event_list)
detectorchecker::plot_events_density(detector_perkinfull_events, 
                                     incl_event_list = incl_event_list, row=1, col=16, adjust=1)
```
...
```{r}
detector_perkinfull_modified <- remove_high_density_cluster(detector_perkinfull, min_pts = 30, eps_adjust = 0.05)
...
This section concerns inferential statistical methods to analyse the state (damaged or intact) of a pixel. General linear models can include a spatial covariates reflecting the location of a pixel relative to the centre, edges or corners of the detector. The spatial covariate could also be in terms of modules (subpanels), such as the pixel's distance to the closest vertical and horizontal boundaries, . Running times are a bit slow, but it should not take longer than a minute for a detector of the size of PerkinElmerFull oder Pilatus. 

This command fits a model that includes the euclidian distance of a pixel to the centre as covariate.
```{r}
glm_pixel_ctr_eucl(detector_pilatus)
```

The next one includes the maximum of the distances of a pixel to the edges of the detector (L1 distance).
```{r}
...
The function glm_pixel_dist_edge_col() fits a glm that includes a pixel's distance from the nearest vertical module edge.
```{r}
...
Finally, this function fits a glm that includes a pixel's distance from the nearest module corner.
```{r}
glm_pixel_dist_corner(detector_pilatus)
```

All these commands are also available on the event level with similar names, just swapping pixel for events in the command name.  This fits a model that includes the euclidian distance of an event to the centre as covariate.
```{r}
glm_events_ctr_eucl(detector_pilatus)
```

It is also possible to visualise the spatial covariates. This is purely for illustration and we do not encourage to do that on a regular basis, because the plotting functions are quite slow.

The command plot_pixel_ctr_eucl() illustrates a pixel's euclidian distance to the centre by colouring its location proportionally to that. The command plot_pixel_ctr_linf() illustrates the minimum of the distances to the two dectector boundaries. The third command visualising spatial covariates related to the whole dectector is based on the euclidian distance to the corners.

```{r}
plot_pixel_dist_corner(detector_pilatus)
```

The covariates containing the euclidian distance of a pixel to the edges of the detectors can be visualised by a colored image of the detector area. These functions are slow as well. The command plot_pixel_dist_edge_col() plots pixels coloured propotionally to their distance to the nearest vertical module boundary, while plot_pixel_dist_edge_row() does the same for horizontal boundaries. The following command uses the distance of a pixel to any of the module edges.
