...
plot_module_pixels(detector = pilatus_detector, col = 4, row = 5, caption = TRUE)
```
...
plot_pixels(detector = detector_perkin, events = FALSE)
plot_module_pixels(detector = detector_perkin, col  =11, row =2)
```
...
plot_module_pixels(detector_exc, col = 1, row = 6)
```
...
detector_perkin <- load_pix_matrix(detector = detector_perkin, file_path = file_path)
plot_module_events(detector_perkin, row = row_, col = col_, incl_event_list = incl_event_list)
```
### Events 
```{r}
