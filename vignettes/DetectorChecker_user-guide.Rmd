...
In the `Pilatus` detectors all horizontal gaps are of the same size, and all vertical gaps are of the same size. `Perkin Elmer` and `Excalibur` detectors have no gaps at all. However, it is possible to create layouts with more irregular gap sizes (see the comments below on user-defined layouts).
...
A visualisation of the layout shows modules and gaps as shown below for two of pre-specified layouts. The default is to plot a title, but we can override this by setting the parameter caption to `FALSE`. The same rule applies for all plots produced using this package.
...
This also works for a user-defined detector if the damaged pixel locations are stored in xml format. (In the example file the dead pixels locations were simulated by an Poisson point pattern with inhomogeneous density.)
...
```{r}
detector_perkinfull_modified <- remove_high_density_cluster(detector_perkinfull, 
                                                            min_pts = 30, eps_adjust = 0.05)
...
This section concerns inferential statistical methods to analyse the state (damaged or intact) of a pixel. General linear models can include a spatial covariates reflecting the location of a pixel relative to the centre, edges or corners of the detector. The spatial covariate could also be in terms of modules (sub-panels), such as the pixel's distance to the closest vertical and horizontal boundaries, . Running times are a bit slow, but it should not take longer than a minute for a detector of the size of PerkinElmerFull or Pilatus. 

This command fits a model that includes the euclidean distance of a pixel to the centre as covariate.
```{r}
...
All these commands are also available on the event level with similar names, just swapping pixel for events in the command name.  This fits a model that includes the euclidean distance of an event to the centre as covariate.
```{r}
...
The command plot_pixel_ctr_eucl() illustrates a pixel's euclidean distance to the centre by colouring its location proportionally to that. The command plot_pixel_ctr_linf() illustrates the minimum of the distances to the two detector boundaries. The third command visualising spatial covariates related to the whole detector is based on the euclidean distance to the corners.

```{r}
plot_pixel_dist_corner(detector_pilatus)
```

The covariates containing the euclidean distance of a pixel to the edges of the detectors can be visualised by a colored image of the detector area. These functions are slow as well. The command plot_pixel_dist_edge_col() plots pixels coloured proportionally to their distance to the nearest vertical module boundary, while plot_pixel_dist_edge_row() does the same for horizontal boundaries. The following command uses the distance of a pixel to any of the module edges.
